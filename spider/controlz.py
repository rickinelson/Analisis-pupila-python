...
condicion1 = cursor.execute("SELECT cuenta, archivo, rt, condicion FROM datos WHERE congruency = 'congruent'  AND response = 'OK' AND condicion = 'V' and sexo = 'F' AND d2 >= '0.3' AND d2 <= '0.5' and archivo not in ('inig_JV','dani_LV','alex_VV','roci_MV')")#azul
condicion1 = cursor.fetchall()

len(condicion1)

condicion2 = cursor.execute("SELECT cuenta, archivo, rt, condicion FROM datos WHERE congruency = 'incongruent' AND response = 'OK' AND condicion = 'T' and sexo = 'F'  AND d2 >= '0.3' AND d2 <= '0.5' and archivo not in ('inig_JT', 'dani_LT', 'alex_VT','roci_MT')") #naranjo
...
def huachimingo (condicion, nombre_archivo):
    ...
    plt.suptitle('Dilatacion pupilar incongruent condicion tactil/vertical- mujeres',fontsize=25)
...
minimo = min(selectiempo)
maximo = max(selectiempo)
...
plt.plot([1,1,1], label="Curva incongruent vertical",markersize = 8, color = 'blue')
plt.plot([1,1,1], label="Curva incongruent tactil", markersize = 8,color = 'orange')
